...
def get_connection():
    return pymysql.connect(host='localhost', user='root', passwd='Qdpfp7xsm2@', database='bus_data')
...
def fetch_route_names(connection, starting_letter):
    query = f"SELECT DISTINCT route_name FROM all_bus_data WHERE route_name LIKE '{starting_letter}%' ORDER BY route_name"
...
def fetch_data(connection, route_name, price_sort_order):
    ...
    query = f"SELECT * FROM all_bus_data WHERE Route_Name = %s ORDER BY Star_Rating DESC, Price {price_sort_order_sql}"
    df = pd.read_sql(query, connection, params=(route_name))
    return df

# Function to filter data based on Star_Rating and Bus_Type
def filter_data(df, selected_ratings, selected_bus_types):
    filtered_df = df[df['star_rating'].isin(selected_ratings) & df['bustype'].isin(selected_bus_types)]
    return filtered_df
                ...
                if selected_route:
                    ...
                    if not data.empty:
                        ...
                        if selected_ratings and selected_bus_types:
                            ...
                            st.write(f"### Filtered Data for Star Rating: {selected_ratings} and bustype: {selected_bus_types}")
